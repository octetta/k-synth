...
int write_wav_from_k(char* filename, double* ptr, ma_uint64 frames, ma_uint32 chans, ma_uint32 sample_rate) {
  ...
  //config = ma_encoder_config_init(ma_encoding_format_mp3, ma_format_f32, chans, sample_rate);
  ...
  // We multiply by chans to ensure space for interleaved data (L, R, L, R...).
...
int main() {
  ...
  printf("ksynth v1.9.0\n");
  printf("exit \\l load | \\p play | \\w wait | \\s save \\v view\n");
